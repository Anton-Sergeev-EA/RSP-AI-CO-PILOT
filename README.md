# RSP AI CO-PILOT
### Предиктивная диагностика оборудования и RAG-ассистент диспетчера (SCADA/HMI)
...
> энергообъектов и к промышленной SCADA-системе заказчика не было.
...
|---|---|---|
| Существующая SCADA-система отображает телеметрию и тревоги, но не объясняет их и не предсказывает развитие | Чат-копайлот на естественном языке объясняет статус агрегата и ссылается на базу эксплуатационных знаний | Меньше нагрузка на дефицитный инженерный персонал, быстрее адаптация новых дежурных |
...
```
                    ...
                    вибрация подшипников + температура (SCADA-система)
                                      │
                                      ▼
                        ┌──────────────────────────┐
                        │  C++ telemetry_engine      │
...
- **Телеметрия синтетическая** — реалистичная симуляция (`backend/ml/simulate_units.py`,
  ...
  телеметрии из промышленной SCADA-системы.
...
1. Пилот на одной станции: подключить реальный экспорт телеметрии из промышленной SCADA-системы.
...
4. Юридически оформить «предиктивную диагностику + AI-копайлот» как отдельный SKU
   поверх существующих лицензий SCADA/HMI.
